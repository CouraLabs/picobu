...
import z from "zod";
import { fetchAsMarkdown, WebfetchToolArgsSchema, webfetchTool } from "./webfetch";
...
describe("webfetch", () => {
  ...
  test("handler delegates to fetchAsMarkdown", async () => {
    const result = await webfetchTool.handler({ url: `${server.url.href}page` });
