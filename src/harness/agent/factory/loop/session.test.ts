...
import { options, type ProviderOptions } from "@libs/options.ts";
import {
  COMPACT_THRESHOLD,
  ...
  SessionSaver,
  shouldCompact,
} from "@harness/agent/factory/loop/session.ts";
import type { LoopConfig, LoopMessage } from "@harness/agent/factory/loop/create-loop.ts";
...
function startMockProvider() {
  ...
  return { provider, requests, close: () => server.stop(true) };
}
