...
export const SessionPrompt = (props: SessionPromptProps) => {
  ...
  const borderColor = () =>
    waitingMode() ? theme().info : queueMode() ? theme().info : steeringMode() ? theme().error : commandOpen() ? theme().accent : theme().border;
  const titleColor = () =>
    waitingMode() ? theme().info : queueMode() ? theme().info : steeringMode() ? theme().error : commandOpen() ? theme().accent : theme().textMuted;
  const title = () =>
    waitingMode()
      ? " Prompt - Waiting "
      : queueMode()
        ? ` Prompt - Queue${queueSuffix()} `
        : steeringMode()
          ? " Prompt Steering "
          : commandOpen()
            ? " Command "
  ...
  const placeholder = () =>
    waitingMode()
      ...
      : queueMode()
        ? "Queued until the run finishes…"
        : steeringMode()
          ? "Steer the running step…"
