...
export const TodoList = (props: TodoListProps) => {
  ...
  const filled = () =>
    props.items.length > 0 ? Math.round((doneCount() / props.items.length) * PROGRESS_WIDTH) : 0
    ...
    <box flexDirection="column" paddingLeft={1}>
      <box flexDirection="row" gap={1}>
        <text fg={theme().primary} selectable={false}>
          {icons.progressFull.repeat(filled())}{icons.progressEmpty.repeat(PROGRESS_WIDTH - filled())}
        </text>
        <text fg={theme().textMuted}>{doneCount()}/{props.items.length}</text>
      </box>
