...
export type HarnessOptions = {
  ...
  /**
   * Tree-wide cap on concurrent spawned sub sessions (depth-inclusive; roots
   * never count). Over-capacity root spawns queue FIFO, nested spawns fail
   * fast. 0 disables spawning. Default 4.
   */
  maxAgents?: number;
};
...
export type OptionsExternal = {
  ...
  /** Optional on disk; defaults to `{ servers: {} }` when unset. */
  mcp?: McpOptions;
}
