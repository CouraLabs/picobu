...
export const ThinkingMessage = ({ markdown, isStreaming, time }: ThinkingMessageProps) => {
  ...
  return (
    <box id="thinking-wrapper" flexDirection="column">
      <text selectable={false} marginLeft={1} fg={mouse || !collapsed ? borderColor : theme.textMuted} onMouseOver={() => setMouse(true)} onMouseOut={() => setMouse(false)} onMouseDown={() => setCollapsed((a) => !a)}>
        { !collapsed ? '┏' : "⏵" }
        { isStreaming ? " Thinking: " + markdown.slice(Math.max(markdown.length - 10, 0), markdown.length).trim() : " Thoughts - " + timeCalc }
      </text>
      { !collapsed ?
        <box id="thinking-wrapper" flexDirection="row" borderStyle="heavy" gap={1} marginLeft={1} paddingX={1} border={["left"]} borderColor={borderColor}>
