          ...
          return (
            <Fragment key={i}>
              <box
                ...
                gap={1}
                flexGrow={1}
                backgroundColor={hoverLine === i || active ? theme.backgroundElement : "transparent"}
                onMouseDown={() => selectLine(i)}
                onMouseOver={() => setHoverLine(i)}
                onMouseOut={() => setHoverLine(null)}
              >
                <box width={4}>
                  <text selectable={false} bg={commented ? theme.warning : "transparent"} fg={commented ? theme.text : theme.textMuted}>
                    {(commented ? icons.edit : '') + String(i + 1).padStart(gutter, " ")}
                  </text>
                </box>
                <box flexGrow={1} flexShrink={1}>
                  <markdown syntaxStyle={syntax} content={line} conceal={true} />
                </box>
              </box>
              {active && (
                <box
                ...
                >
                  ...
                  <box flexDirection="row" gap={1}>
                    <Button variant="info" bordered={false} onPress={saveComment}>
                      Save comment
                    </Button>
                    {commented && (
...
export const PlanReviewFooter = ({ partKey, onPrompt }: { partKey: string; onPrompt: (text?: string, files?: PromptFile[]) => void }) => {
  ...
  return (
    <box flexDirection="row" gap={1}>
      <Button bordered={false} variant="error" onPress={reject}>
        Reject
      </Button>
      <Button bordered={false} variant="error" onPress={dismiss}>
        Dismiss
      </Button>
      <Button bordered={false} variant="success" onPress={approve}>
