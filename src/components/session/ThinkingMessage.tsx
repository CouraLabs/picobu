...
/**
 * Rendered model reasoning, styled muted to sit below the user/assistant
 * voices. Memoized: all props are primitives, so per-token parent re-renders
 * skip the settled `<markdown>` (no re-parse/re-conceal flicker).
 */
export const ThinkingMessage = memo(({ markdown, isStreaming, time }: ThinkingMessageProps) => {
  ...
  return (
    <box id="thinking-wrapper" flexDirection="column">
      <text selectable={false} flexWrap="no-wrap" height={1} fg={mouse || !collapsed ? theme.secondary : theme.textMuted} onMouseOver={() => setMouse(true)} onMouseOut={() => setMouse(false)} onMouseDown={() => setCollapsed((a) => !a)}>
        { !collapsed ? icons.collapse : icons.play }
