...
export type ModelStatusBarProps = {
  ...
  /** USD cost from run metadata (computed in the loop); falls back to the
   * billing-table computation below (e.g. for resumed sessions). */
  cost: number | null;
  elapsedSec: number;
  ttftMs: number | null;
...
export const ModelStatusBar = ({
  ...
  // Prefer the cost computed at the loop (per-step metadata); compute from the
  // billing table when metadata is absent (e.g. resumed sessions).
  const cost = metadataCost ?? (billing
