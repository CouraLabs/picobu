    ...
    const promise = (async () => {
      try {
        ...
        if (opts.transportFactory) {
          transport = opts.transportFactory(server)
        } else if (server.type === 'stdio') {
          const command = server.command
          if (!command) throw new Error(`MCP server "${server.id}" (stdio) requires "command"`)
          ...
          if (!url) throw new Error(`MCP server "${server.id}" requires "url"`)
          transport = {
            type: server.type,
            url,
            ...(headers ? { headers } : {}),
