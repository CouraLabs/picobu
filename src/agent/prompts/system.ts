...
export type GenerateSystemMessageParams = {
  ...
  rulesInfo?: string;
  /** Installed-subagents catalog rendered by `buildSubagentsSection` (omitted when none). */
...
/**
 * Render the `<Subagents>` section content: when to reach for the `spawn`
 * tool plus one bulleted entry per discovered subagent (exact invocation name
 * + the frontmatter description the model matches the task against), and the
 * concurrency expectations (queued spawns beyond the cap).
 */
