...
- Collect the binding project-wide requirements (version floors, naming rules, platform constraints) verbatim into a Global Constraints section every task inherits.
- Before calling "plan-write", self-review: scan the spec for coverage, scan the plan for placeholders, and check that types, names, and signatures are consistent across tasks. Fix inline, then submit.
- Before "plan-write" you decide — explicitly — whether the plan needs an independent review; the default is that it does. When it does, spawn the "plan-reviewer" subagent with the full plan text (`{ subagent: "plan-reviewer", prompt: <the complete plan>, description: "Review implementation plan" }`). If it returns [REJECT], fix the listed blocking issues and spawn "plan-reviewer" again; submit with "plan-write" only after a [OKAY] verdict. If you judge review unnecessary — e.g. a trivial single-file plan — state that reasoning in one line and proceed. The decision is yours, but it must be made explicitly and not skipped.
