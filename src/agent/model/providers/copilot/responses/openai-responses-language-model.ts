...
import { combineHeaders, createEventSourceResponseHandler, createJsonResponseHandler, generateId, type ParseResult, parseProviderOptions, postJsonToApi } from '@ai-sdk/provider-utils'
import { z } from 'zod'
...
import type { codeInterpreterInputSchema, codeInterpreterOutputSchema } from './tool/code-interpreter.ts'
import type { fileSearchOutputSchema } from './tool/file-search.ts'
import type { imageGenerationOutputSchema } from './tool/image-generation.ts'
import type { localShellInputSchema } from './tool/local-shell.ts'
...
const codeInterpreterCallItem = z.object({
  ...
  outputs: z.array(z.discriminatedUnion('type', [z.object({ type: z.literal('logs'), logs: z.string() }), z.object({ type: z.literal('image'), url: z.string() })])).nullable(),
})
...
export class OpenAIResponsesLanguageModel implements LanguageModelV4 {
  ...
  private async getArgs({
    ...
    const reasoningSummary = openaiOptions?.reasoningSummary ?? (reasoningEffort != null ? 'auto' : undefined)
    ...
    const topLogprobs = typeof openaiOptions?.logprobs === 'number' ? openaiOptions?.logprobs : openaiOptions?.logprobs === true ? TOP_LOGPROBS_MAX : undefined
    ...
    const webSearchToolName = (
      tools?.find((tool) => tool.type === 'provider' && (tool.id === 'openai.web_search' || tool.id === 'openai.web_search_preview')) as LanguageModelV4ProviderTool | undefined
    )?.name
  ...
  async doGenerate(options: LanguageModelV4CallOptions) {
    ...
    const copilotMetadata: JSONObject = {
      responseId: response.id,
    }

    if (logprobs.length > 0) {
      copilotMetadata.logprobs = logprobs
    }

    if (typeof response.service_tier === 'string') {
      copilotMetadata.serviceTier = response.service_tier
    }
    ...
    return {
      ...
      usage: {
        inputTokens: {
          ...
          noCache: response.usage.input_tokens_details?.cached_tokens != null ? response.usage.input_tokens - response.usage.input_tokens_details.cached_tokens : undefined,
  ...
  async doStream(options: LanguageModelV4CallOptions) {
    ...
    return {
      stream: response.pipeThrough(
        new TransformStream<ParseResult<z.infer<typeof openaiResponsesChunkSchema>>, LanguageModelV4StreamPart>({
          start(controller) {
          ...
          transform(chunk, controller) {
            ...
            if (isResponseOutputItemAddedChunk(value)) {
              ...
              const toolCall = ongoingToolCalls[value.output_index]
              const containerId = toolCall?.codeInterpreter?.containerId

              if (toolCall != null && containerId != null) {
                controller.enqueue({
                  type: 'tool-input-delta',
                  id: toolCall.toolCallId,
                  delta: '"}',
          ...
          flush(controller) {
            ...
            if (serviceTier !== undefined) {
              copilotMetadata.serviceTier = serviceTier
            }
            ...
            controller.enqueue({
              ...
              usage: {
                inputTokens: {
                  ...
                  noCache: usage.inputTokens != null && usage.cachedInputTokens != null ? usage.inputTokens - usage.cachedInputTokens : undefined,
...
function isTextDeltaChunk(chunk: z.infer<typeof openaiResponsesChunkSchema>): chunk is z.infer<typeof textDeltaChunkSchema> {
  return chunk.type === 'response.output_text.delta'
}

function isResponseOutputItemDoneChunk(chunk: z.infer<typeof openaiResponsesChunkSchema>): chunk is z.infer<typeof responseOutputItemDoneSchema> {
  return chunk.type === 'response.output_item.done'
}

function isResponseOutputItemDoneReasoningChunk(chunk: z.infer<typeof openaiResponsesChunkSchema>): chunk is z.infer<typeof responseOutputItemDoneSchema> & {
  item: ExtractByType<z.infer<typeof responseOutputItemDoneSchema>['item'], 'reasoning'>
} {
  return isResponseOutputItemDoneChunk(chunk) && chunk.item.type === 'reasoning'
}

function isResponseFinishedChunk(chunk: z.infer<typeof openaiResponsesChunkSchema>): chunk is z.infer<typeof responseFinishedChunkSchema> {
  return chunk.type === 'response.completed' || chunk.type === 'response.incomplete'
}

function isResponseCreatedChunk(chunk: z.infer<typeof openaiResponsesChunkSchema>): chunk is z.infer<typeof responseCreatedChunkSchema> {
  return chunk.type === 'response.created'
}

function isResponseFunctionCallArgumentsDeltaChunk(chunk: z.infer<typeof openaiResponsesChunkSchema>): chunk is z.infer<typeof responseFunctionCallArgumentsDeltaSchema> {
  return chunk.type === 'response.function_call_arguments.delta'
}

function isResponseImageGenerationCallPartialImageChunk(chunk: z.infer<typeof openaiResponsesChunkSchema>): chunk is z.infer<typeof responseImageGenerationCallPartialImageSchema> {
  return chunk.type === 'response.image_generation_call.partial_image'
}

function isResponseCodeInterpreterCallCodeDeltaChunk(chunk: z.infer<typeof openaiResponsesChunkSchema>): chunk is z.infer<typeof responseCodeInterpreterCallCodeDeltaSchema> {
  return chunk.type === 'response.code_interpreter_call_code.delta'
}

function isResponseCodeInterpreterCallCodeDoneChunk(chunk: z.infer<typeof openaiResponsesChunkSchema>): chunk is z.infer<typeof responseCodeInterpreterCallCodeDoneSchema> {
  return chunk.type === 'response.code_interpreter_call_code.done'
}

function isResponseOutputItemAddedChunk(chunk: z.infer<typeof openaiResponsesChunkSchema>): chunk is z.infer<typeof responseOutputItemAddedSchema> {
  return chunk.type === 'response.output_item.added'
}

function isResponseOutputItemAddedReasoningChunk(chunk: z.infer<typeof openaiResponsesChunkSchema>): chunk is z.infer<typeof responseOutputItemAddedSchema> & {
  item: ExtractByType<z.infer<typeof responseOutputItemAddedSchema>['item'], 'reasoning'>
} {
  return isResponseOutputItemAddedChunk(chunk) && chunk.item.type === 'reasoning'
}

function isResponseAnnotationAddedChunk(chunk: z.infer<typeof openaiResponsesChunkSchema>): chunk is z.infer<typeof responseAnnotationAddedSchema> {
  return chunk.type === 'response.output_text.annotation.added'
}

function isResponseReasoningSummaryPartAddedChunk(chunk: z.infer<typeof openaiResponsesChunkSchema>): chunk is z.infer<typeof responseReasoningSummaryPartAddedSchema> {
  return chunk.type === 'response.reasoning_summary_part.added'
}

function isResponseReasoningSummaryTextDeltaChunk(chunk: z.infer<typeof openaiResponsesChunkSchema>): chunk is z.infer<typeof responseReasoningSummaryTextDeltaSchema> {
