...
import { DirectChatTransport, type InferUITools, isStepCount, type LanguageModel, ToolLoopAgent, type UIMessage } from "ai";
...
export function createLoop(getConfig: () => LoopConfig): Loop {
  ...
  const mcpInfo = async (agentDef: { tools: string[] }): Promise<string> => {
    ...
    return snapshots
      .map((snapshot) =>
        snapshot.connected && snapshot.tools.length ? renderMcpServerToolsInfo(snapshot.id, snapshot.instructions ?? snapshot.serverInstructions, snapshot.tools) : "",
      )
      .filter(Boolean)
      .join("\n\n");
  };
