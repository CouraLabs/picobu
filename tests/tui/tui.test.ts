...
describe('resolveTheme', () => {
  const baseTheme = (): ThemeJson => {
    const found = allThemes().monochrome
    if (!found) throw new Error('monochrome theme missing')
    return found
  }
  test('preserves dark/light variants', () => {
    const variant: ThemeJson = { ...baseTheme(), theme: { ...baseTheme().theme, primary: { dark: '#000000', light: '#ffffff' } } }
  ...
  test('resolves defs references', () => {
    const ref: ThemeJson = { defs: { ...baseTheme().defs, brand: '#ff0000' }, theme: { ...baseTheme().theme, primary: 'brand' } }
  ...
  test('rejects circular and missing references', () => {
    const circular: ThemeJson = { defs: { a: 'b', b: 'a' }, theme: { ...baseTheme().theme, primary: 'a' } }
    expect(() => resolveTheme(circular, 'dark')).toThrow('Circular color reference')
    const missing: ThemeJson = { theme: { ...baseTheme().theme, primary: 'missing-color' } }
  ...
  test('supports transparency and opacity defaults', () => {
    const clear: ThemeJson = { ...baseTheme(), theme: { ...baseTheme().theme, background: 'transparent' } }
    expect(resolveTheme(clear, 'dark').background.a).toBe(0)
    expect(resolveTheme(baseTheme(), 'dark').thinkingOpacity).toBe(0.6)
    const custom: ThemeJson = { ...baseTheme(), theme: { ...baseTheme().theme, thinkingOpacity: 0.9 } }
  ...
  test('falls back backgroundMenu to backgroundElement', () => {
    ...
    void _dropped
    const resolved = resolveTheme({ ...baseTheme(), theme: rest }, 'dark')
