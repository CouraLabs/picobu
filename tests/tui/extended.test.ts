...
describe('theme state', () => {
  test('lists sorted themes with seeded default', () => {
    ...
    expect(snapshot.theme.hasMonochrome).toBe(true)
    expect(snapshot.theme.hasPicobu).toBe(true)
    expect(snapshot.theme.sorted).toBe(true)
    expect(snapshot.theme.initial).toEqual({ name: 'picobu', variant: 'dark' })
  })
  ...
  test('setTheme falls back to picobu for unknown themes', () => {
    expect(snapshot.theme.unknownThemeError).toBe('picobu')
  })
})
