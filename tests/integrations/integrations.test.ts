...
describe('renderMcpToolInfo', () => {
  test('contains compact name and description', () => {
    const out = renderMcpToolInfo('mcp_srv_search', 'Search things')
    expect(out).toContain('mcp_srv_search')
    expect(out).toContain('Search things')
    expect(out).not.toContain('```json')
  })
  test('falls back when description is missing', () => {
    const out = renderMcpToolInfo('mcp_srv_search', undefined)
    expect(out).toContain('mcp_srv_search')
...
describe('mcp auth redirect', () => {
  ...
  test('allows same origin authorization server', () => {
    ...
    if (!validate) throw new Error('missing validateAuthorizationServerURL')
    expect(() => validate('https://api.example.com/mcp', 'https://api.example.com/auth')).not.toThrow()
  })
  test('enforces https for remote authorization servers', () => {
    const { provider } = createMcpAuthProvider({ id: 't', type: 'http', url: 'https://api.example.com/mcp' })
    const validate = provider.validateAuthorizationServerURL
    if (!validate) throw new Error('missing validateAuthorizationServerURL')
    expect(() => validate('https://api.example.com/mcp', 'http://api.example.com/auth')).toThrow('https required')
  })
  test('allows same root subdomain authorization server', () => {
    const { provider } = createMcpAuthProvider({ id: 't', type: 'http', url: 'https://api.example.com/mcp' })
    const validate = provider.validateAuthorizationServerURL
    if (!validate) throw new Error('missing validateAuthorizationServerURL')
    expect(() => validate('https://api.example.com/mcp', 'https://auth.example.com/oauth')).not.toThrow()
  })
  test('rejects unrelated authorization server origin', () => {
    const { provider } = createMcpAuthProvider({ id: 't', type: 'http', url: 'https://api.example.com/mcp' })
    const validate = provider.validateAuthorizationServerURL
    if (!validate) throw new Error('missing validateAuthorizationServerURL')
    expect(() => validate('https://api.example.com/mcp', 'https://evil.com/oauth')).toThrow('unexpected OAuth authorization server')
  })
  test('allows same origin http for localhost servers', () => {
    ...
    const validate = provider.validateAuthorizationServerURL
    if (!validate) throw new Error('missing validateAuthorizationServerURL')
    expect(() => validate('http://localhost:3000/mcp', 'http://localhost:3000/auth')).not.toThrow()
  })
  test('rejects unparsable authorization server url', () => {
    const { provider } = createMcpAuthProvider({ id: 't', type: 'http', url: 'https://api.example.com/mcp' })
    const validate = provider.validateAuthorizationServerURL
    if (!validate) throw new Error('missing validateAuthorizationServerURL')
    expect(() => validate('https://api.example.com/mcp', 'nota-a-url')).toThrow()
  })
})
