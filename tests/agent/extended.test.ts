...
describe("computeCostSplit", () => {
  ...
  test("clamps negatives and cache overcount", () => {
    const split = computeCostSplit(
      { inputTokens: -5, outputTokens: -2, cacheReadTokens: -1, cacheWriteTokens: -1 },
      { input: 3, output: 6, cacheRead: 1, cacheWrite: 2 },
    );
...
describe("agent prompt texts", () => {
  ...
  test("generateSystemMessage substitutes params and appends sections", () => {
    ...
    expect(byKey.get("System Guideless")).toContain("APPENDIX-MARK");
...
describe("prompt embeds", () => {
  ...
  test("resolvePrompt replaces text and file tokens", () => {
    const resolved = resolvePrompt(
      "A [T#1 pasted] B [F#2 file] C [T#9 missing] D",
      { "T#1": "HELLO" },
      { "F#2": { mimeType: "image/png", filename: "pic.png", dataUrl: "data:image/png;base64,xx" } },
    );
