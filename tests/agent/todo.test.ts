...
describe('createTodoTool', () => {
  ...
  test('ins with an empty list is rejected', async () => {
    await expect(tool().handler({ actionType: 'ins', action: { ins: [] } })).rejects.toThrow("todo 'ins' requires action.ins to list at least one item")
  })
  ...
  test('upd index out of range is rejected and leaves the file untouched', async () => {
    ...
    await expect(tool().handler({ actionType: 'upd', action: { upd: { index: 5, item: item('x') } } })).rejects.toThrow('todo index 5 out of range (1 item(s))')
    expect(await readFile(todoPath, 'utf8')).toBe(before)
  })

  test('del index out of range is rejected', async () => {
    await tool().handler({ actionType: 'ins', action: { ins: [item('a')] } })
    await expect(tool().handler({ actionType: 'del', action: { del: { index: 1 } } })).rejects.toThrow('todo index 1 out of range (1 item(s))')
  })

  test('missing action payloads are rejected', async () => {
    await expect(tool().handler({ actionType: 'upd', action: {} })).rejects.toThrow("todo 'upd' requires action.upd { index, item }")
    await expect(tool().handler({ actionType: 'del', action: {} })).rejects.toThrow("todo 'del' requires action.del { index }")
  })

  test('a corrupt todo file is reported instead of silently reset', async () => {
    await writeFile(todoPath, 'not json')
    await expect(tool().handler({ actionType: 'ins', action: { ins: [item('a')] } })).rejects.toThrow(`Corrupt todo file at ${todoPath}`)
  })

  test('args schema rejects unknown action types and non-integer indices', () => {
    expect(tool().parameters.safeParse({ actionType: 'merge', action: {} }).success).toBe(false)
    expect(
