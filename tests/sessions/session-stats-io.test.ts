...
describe('session stats io', () => {
  ...
  test('write then read round trip preserves totals', async () => {
    ...
    store.handleStepEnd({ usage: { ...emptyUsage(), inputTokens: 10, totalTokens: 10 }, performance: {} as never, warnings: undefined, response: {}, finishReason: 'stop' })
    store.handleEnd({ usage: { ...emptyUsage(), inputTokens: 10, totalTokens: 10 }, finishReason: 'stop' })
    ...
    expect(loaded?.tokenTotals).toEqual({ inputTokens: 10, outputTokens: 0 })
    expect(loaded?.usage.inputTokens).toBe(10)
    ...
    resumed.handleEnd({ usage: { ...emptyUsage(), inputTokens: 5, totalTokens: 5 }, finishReason: 'stop' })
    expect(resumed.get().usage.inputTokens).toBe(10)
  })
  test('corrupt file reads as undefined', async () => {
    ...
    expect(isLoopStats({ usage: { inputTokens: 0, outputTokens: 0, totalTokens: 0 }, cost: { input: 0, output: 0, cache: 0, total: 0 } })).toBe(true)
    expect(isLoopStats({ total: { usage: {}, cost: { input: 0 } } })).toBe(false)
