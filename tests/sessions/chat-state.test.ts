...
describe('createHeadlessChatState snapshots', () => {
  test('stored messages get fresh identities on every write', () => {
    ...
    const producedFirst = produced[0]
    if (!producedFirst) throw new Error('missing produced message')
    const producedPart = producedFirst.parts[0]
    if (!producedPart) throw new Error('missing produced part')
    ;(producedPart as Record<string, unknown>).output = { status: 'pending' }
    ;(producedPart as Record<string, unknown>).state = 'output-available'
    state.messages = produced
  ...
  test('later producer mutations do not leak into stored state', () => {
    ...
    expect((state.messages[0]?.parts[0] as Record<string, unknown> | undefined)?.state).toBe('input-available')
  })
  test('replaceMessage stores an isolated copy', () => {
    ...
    expect((state.messages[0]?.parts[0] as Record<string, unknown> | undefined)?.output).toBeUndefined()
  })
})
