...
describe('actions validation', () => {
  ...
  test('today tasks roundtrip in tmp system dir', async () => {
    ...
    try {
      ...
      if (!firstItem) throw new Error('missing today item')
      expect(await removeTodayTask(firstItem.id)).toContain('removed')
...
describe('wwp-tools schemas', () => {
  ...
  test('wwp-msg params require phone and message strings', () => {
    ...
    if (!msg) throw new Error('missing wwp-msg tool')
  ...
  test('wwp-today params require text', () => {
    ...
    if (!today) throw new Error('missing wwp-today tool')
