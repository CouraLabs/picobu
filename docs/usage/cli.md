...
`picobu` is the single entry point: it bootstraps the app (autoloads providers, refreshes OAuth tokens, connects WhatsApp when enabled) and then opens the TUI or runs a subcommand. `bun dev` from a clone does the same as `picobu`.
...
| `--session [id]` | Open the TUI resuming a session (most recent if no id given). |
